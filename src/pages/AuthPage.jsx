...
const SEXES = [
  { id: 'homme',  dbVal: 'MALE',   fr: 'Homme',              en: 'Male'              },
  { id: 'femme',  dbVal: 'FEMALE', fr: 'Femme',              en: 'Female'            },
  { id: 'neutre', dbVal: 'OTHER',  fr: 'Ne se prononce pas', en: 'Prefer not to say' },
]
...
export default function AuthPage({ onDone }) {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      if (!userId) throw new Error(t('Erreur création compte', 'Account creation error'))

      const birthYear = new Date(dob).getFullYear()
      const age = new Date().getFullYear() - birthYear

      // Convertir sexe (homme/femme/neutre) → valeur DB (MALE/FEMALE/OTHER)
      const sexeObj = SEXES.find(s => s.id === sexe)
      const genderDb = sexeObj?.dbVal || 'OTHER'
      ...
      const { error: profileErr } = await supabase.from('profiles').upsert({
        ...
        email,
        dob,
        sexe,                // colonne sexe garde 'homme'/'femme'/'neutre' (contrainte: homme/femme/neutre)
        gender:     genderDb, // colonne gender attend MALE/FEMALE/OTHER
        age,
        height:     parseFloat(taille.replace(',', '.')),
        weight:     parseFloat(poids.replace(',', '.')),
      ...
      await supabase.from('weight_history').insert({
        user_id:     userId,
        value:       parseFloat(poids.replace(',', '.')),
  ...
  const fa = e => e.target.style.borderColor = 'var(--acc)'
      ...
      <div style={{
        ...
        {mode === 'login' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {error && (
              <p style={{ fontSize: 13, color: 'var(--err)', textAlign: 'center' }}>
                {error}
              </p>
            )}
            <button onClick={handleLogin} disabled={loading} style={btnAcc}>
        ...
        {mode === 'signup' && step === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label style={lbl}>{t('Prénom', 'First name')} *</label>
              <input
                type="text"
            ...
            <button onClick={handleNext} style={btnAcc}>
        ...
        {mode === 'signup' && step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={lbl}>{t('Date de naissance', 'Date of birth')} *</label>
              <input
                type="date"
                value={dob}
            ...
            <div>
              <label style={lbl}>{t('Sexe', 'Sex')} *</label>
              <div style={{ display: 'flex', gap: 8 }}>
                {SEXES.map(s => (
                  <button
                    key={s.id}
                    onClick={() => setSexe(s.id)}
                    style={{
                      flex: 1,
                      padding: '11px 4px',
                      borderRadius: 12,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                <label style={lbl}>{t('Taille (cm)', 'Height (cm)')} *</label>
                <input
                  type="text"
                  inputMode="decimal"
                  value={taille}
              ...
              <div>
                <label style={lbl}>{t('Poids (kg)', 'Weight (kg)')} *</label>
                <input
                  type="text"
                  inputMode="decimal"
                  value={poids}
        ...
        {mode === 'signup' && step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={lbl}>{t('Objectif', 'Goal')} *</label>
            ...
            <div>
              <label style={lbl}>{t('Niveau', 'Level')} *</label>
              <div style={{ display: 'flex', gap: 8 }}>
                {LEVELS.map(l => (
                  <button
                    key={l.id}
            ...
            <div>
              <label style={lbl}>{t('Séances / semaine', 'Sessions / week')}</label>
              <div style={{ display: 'flex', gap: 8 }}>
                {FREQ.map(f => (
                  <button
                    key={f}
