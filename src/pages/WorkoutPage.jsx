...
const FOCUS_COLOR = { PUSH:'acc', PULL:'blue', LEGS:'green', FULL:'gray', UPPER:'blue', LOWER:'green', CUSTOM:'gray' }

export default function WorkoutPage({ onBack, onNew }) {
  const { current, setCurrent, saveSession } = useWorkout()
  ...
  if (!current) {
    return (
      <div style={{
        ...
        <p style={{ fontSize: 15, color: 'var(--txt-sub)' }}>
          {lang === 'fr' ? 'Aucune séance en cours.' : 'No active workout.'}
        </p>
        <Button label={lang === 'fr' ? 'Générer une séance' : 'Generate workout'} onClick={onNew} />
      </div>
    )
  }
  ...
  const handleReplace = (ex) => {
    ...
    const newMain = current.main.map(e =>
      e.id === ex.id ? { ...replacement, order: e.order, progress_note: e.progress_note } : e
    )
  ...
  const isCustom = current.focus === 'CUSTOM'

  const tabs = [
    { id: 'warmup',    label: t('warmup'),    show: !isCustom && current.warmup?.length > 0 },
    { id: 'main',      label: t('main'),       show: true },
    { id: 'finisher',  label: t('finisher'),   show: !isCustom && !!current.finisher },
    { id: 'stretches', label: t('stretches'),  show: !isCustom && current.cooldown?.length > 0 },
  ].filter(tb => tb.show)

  const activeTab = tabs.find(tb => tb.id === tab) ? tab : 'main'

  const workoutTitle = isCustom
    ? (current.custom_name || (lang === 'fr' ? 'Séance custom' : 'Custom workout'))
    : `${lang === 'fr' ? 'Séance' : 'Workout'} ${current.focus}`
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--bg-base)', paddingBottom: 120 }}>
      ...
      <div style={{ padding: '16px 18px 0' }}>
        ...
        <div className="fade-up" style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--txt-muted)' }}>
              {current.exerciseCount} {t('exos')} · {current.totalSets} {t('series')}
              {current.estimatedDuration ? ` · ~${current.estimatedDuration} min` : ''}
            </span>
          </div>
          <h1 style={{
            fontFamily: "'Bebas Neue', sans-serif",
            fontSize: 34, color: 'var(--txt)',
          ...
          <div className="fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {current.warmup.map((item, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  width: 32, height: 32, background: 'var(--acc-dim)',
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span style={{
                      fontSize: 11, color: 'var(--acc-txt)', background: 'var(--acc-dim)',
          ...
          <div className="fade-up">
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 12 }}>
                ...
                <h3 style={{ fontSize: 17, fontWeight: 800, color: 'var(--txt)' }}>
                  {current.finisher.name}
                </h3>
                {current.finisher.rounds && (
                  <span style={{
                    background: 'rgba(255,159,10,0.12)', color: 'var(--warn)',
              ...
              {current.finisher.note && (
                <p style={{ fontSize: 12, color: 'var(--txt-sub)', marginBottom: 14 }}>
                  {current.finisher.note}
                </p>
              )}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {current.finisher.items?.map((item, i) => (
                  <div key={i} style={{
                    ...
                    padding: '10px 14px', background: 'var(--surface-up)', borderRadius: 12,
                  }}>
        ...
        {activeTab === 'stretches' && (
          <div className="fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{
              padding: '10px 14px', background: 'rgba(48,209,88,0.08)',
              borderRadius: 12, display: 'flex', gap: 8, marginBottom: 4,
            }}>
              ...
              <p style={{ fontSize: 12, color: 'var(--ok)', lineHeight: 1.5 }}>
                {lang === 'fr'
                  ...
                  : 'These stretches target the muscles worked today.'}
              </p>
            </div>
            {current.cooldown.map((item, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  width: 32, height: 32, background: 'rgba(48,209,88,0.10)',
                  borderRadius: 10, display: 'flex', alignItems: 'center',
                  justifyContent: 'center', flexShrink: 0,
                }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--txt)' }}>{item.name}</span>
                    <span style={{
                      fontSize: 11, color: 'var(--ok)', background: 'rgba(48,209,88,0.10)',
                      padding: '2px 8px', borderRadius: 999, fontWeight: 700,
                    }}>
                      {item.duration}
                    </span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 24 }}>
          {!saved ? (
            ...
            <>
              <div style={{
                ...
                background: 'rgba(48,209,88,0.10)', borderRadius: 16,
          ...
          <Button
            label={t('newW')} full variant="ghost" onClick={onNew}
